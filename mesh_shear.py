...
with (prime.launch_prime(n_procs=16, timeout=60) as prime_client):
    ...
    splitter_control = model.control_data.create_size_control(prime.SizingType.CURVATURE)
    ...
    wall_faces = part.get_topo_faces_of_label_name_pattern(f"!*internal",
    ...
    print(prime.AutoMesh(model).mesh(part_id=model.parts[0].id, automesh_params=automesh_params))
